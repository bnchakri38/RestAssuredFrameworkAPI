...
public class UpdateUserTest extends BaseTest{
	...
	public void updateAUserTest() {
		...
		// 3. Update the user using the same userId
			...
			userBody.put("name", "Chakri Automation");
			userBody.put("status", "inactive");
		
		Response responsePut = restClient.put(BASE_URL_GOREST, GOREST_USERS_ENDPOINT+"/"+userId, userBody, null, null, AuthType.BEARER_TOKEN, ContentType.JSON);
		Assert.assertTrue(responsePut.statusLine().contains("OK"));
		Assert.assertEquals(responsePut.jsonPath().getString("id"), userId);
		
		Assert.assertEquals(responsePut.jsonPath().getString("name"), "Chakri Automation");
		...
		Assert.assertEquals(responseGet.jsonPath().getString("name"), "Chakri Automation");
